...
const WeatherDataDisplay = React.memo(({ weatherData, onForecastTime }) => {
  ...
  const getCurrentOrClosestTimeData = (times) => {
    const now = new Date();
    ...
    for (let time of times) {
      ...
      if (startTime > now) {
        closestFutureTime = closestFutureTime || time;
        break;
      }
      latestPastTime = time;
    }

    return closestFutureTime || latestPastTime;
  };
  ...
  useEffect(() => {
    if (sortedWeatherData.length > 0) {
      for (let location of sortedWeatherData) {
        ...
        if (wxElement && wxElement.time.length > 0) {
          const relevantTimeData = getCurrentOrClosestTimeData(wxElement.time);
          if (relevantTimeData) {
            onForecastTime({
              startTime: relevantTimeData.startTime,
              endTime: relevantTimeData.endTime,
            });
            break;
          }
        }
  ...
  return (
    <div className="data-container">
      {sortedWeatherData.map((location, index) => {
        ...
        const relevantWxData = getCurrentOrClosestTimeData(
          wxElement.time
        )?.parameter;
        const minTemp = getCurrentOrClosestTimeData(minTempElement?.time)
          ?.parameter?.parameterName;
        const maxTemp = getCurrentOrClosestTimeData(maxTempElement?.time)
          ?.parameter?.parameterName;
        const ci = getCurrentOrClosestTimeData(ciElement?.time)?.parameter
          ?.parameterName;
        const pop = getCurrentOrClosestTimeData(popElement?.time)?.parameter
          ?.parameterName;
        const wxValue = relevantWxData?.parameterValue;
        ...
        return (
          <div key={index} className="location">
            ...
            <img src={iconSrc} alt={relevantWxData?.parameterName} />

            <p>天氣現象 ：{relevantWxData?.parameterName}</p>
            <p>降雨機率 ：{pop} %</p>
            <p>體感狀況 ：{ci}</p>
            <p>
              氣溫範圍 ：{minTemp} ℃ ~ {maxTemp} ℃
